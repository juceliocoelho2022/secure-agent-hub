...
public class RuleBasedAgentPlanner implements AgentPlanner {
    ...
    public AgentPlan plan(String prompt) {
        ...
        if (normalized.contains("bloque") || normalized.contains("fraude")) {
            return new AgentPlan("blockCard", "Potential fraud detected; proposes card block");
        }
        if (normalized.contains("transação") || normalized.contains("transacao")) {
            return new AgentPlan("getTransaction", "Retrieves transaction data for analysis");
        }
        return new AgentPlan("calculateRisk", "Calculates a baseline risk score");
    }
}
