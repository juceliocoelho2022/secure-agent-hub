...
## Request contract
`POST /api/v1/agents/executions` keeps `agent` and `prompt` and adds an optional `context` object carrying trusted facts such as transaction id, amount, country, usual country, hour, rapid retry and known device.
...
No change to RBAC, HITL, Policy Engine authority, planner selection, or controlled tool execution. `blockCard` remains human-approval protected.
